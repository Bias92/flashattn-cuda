// ============================================================
// flash_attn_fa3.cu -- 3rd-gen scratch FlashAttention forward
//
...
// Host launchers
// ============================================================
static std::pair<torch::Tensor, torch::Tensor> fa3_forward_impl(
    torch::Tensor Q, torch::Tensor K, torch::Tensor V)
{
    ...
    fa3_fwd_kernel<HD><<<grid, block, 0, stream>>>(
...
std::vector<torch::Tensor> fa3_forward(torch::Tensor Q, torch::Tensor K, torch::Tensor V) {
    auto [O, L] = fa3_forward_impl(Q, K, V);
    return {O, L};
}

torch::Tensor fa3_forward_only(torch::Tensor Q, torch::Tensor K, torch::Tensor V) {
    auto [O, L] = fa3_forward_impl(Q, K, V);
    return O;
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &fa3_forward, "FA3 forward (mma.sync, register-resident softmax): returns O half, L float");
    ...
    m.def("forward_only", &fa3_forward_only, "FA3 forward returning O only (L still computed internally, parity with SDPA)");
}
