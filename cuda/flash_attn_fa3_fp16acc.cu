// ============================================================
// flash_attn_fa3_fp16acc.cu -- ABLATION: fp16-accumulate QK^T
//
// Identical to flash_attn_fa3_db_full.cu except the QK^T mma uses
...
// Host launchers
// ============================================================
static std::pair<torch::Tensor, torch::Tensor> fa3fp16a_forward_impl(
    torch::Tensor Q, torch::Tensor K, torch::Tensor V, bool want_L)
{
    ...
    if (want_L) {
        if (full_tiles) launch(fa3fp16a_fwd_kernel<HD, true, true>);
        else            launch(fa3fp16a_fwd_kernel<HD, true, false>);
    } else {
        if (full_tiles) launch(fa3fp16a_fwd_kernel<HD, false, true>);
        else            launch(fa3fp16a_fwd_kernel<HD, false, false>);
    }
...
std::vector<torch::Tensor> fa3fp16a_forward(torch::Tensor Q, torch::Tensor K, torch::Tensor V) {
    auto [O, L] = fa3fp16a_forward_impl(Q, K, V, /*want_L=*/true);
    return {O, L};
}

torch::Tensor fa3fp16a_forward_only(torch::Tensor Q, torch::Tensor K, torch::Tensor V) {
    auto [O, L] = fa3fp16a_forward_impl(Q, K, V, /*want_L=*/false);
    return O;
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &fa3fp16a_forward, "ABLATION: fp16-accumulate QK forward (returns O half, L float)");
    m.def("forward_only", &fa3fp16a_forward_only, "ABLATION: fp16-accumulate QK forward, true O-only");
}
