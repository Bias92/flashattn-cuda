// ============================================================
// flash_attn_fa3_swz.cu -- race path + PAD=0 XOR swizzle, 6 blocks/SM
//
// Same math/pipeline as flash_attn_fa3_race.cu (split-loop, N_STATIC,
// cp.async .ca/.cg macro). Two changes:
//   1. PAD 8 -> 0: shared row stride 64 halves = 128B. Total shared
//      drops 18432B -> 16384B, clearing the 6-blocks/SM cap (<=17066B).
//      Bank conflicts from the power-of-two stride are removed with the
//      standard 128B XOR swizzle: within a row of eight 16B chunks,
//        physical_chunk = logical_chunk XOR (row & 7)
//      Applied consistently at every producer/consumer: cp.async dst,
//      Q staging stores, and all ldmatrix operands. Since 16B-chunk
//      indices are 3-bit, the XOR is exact; row&7 is invariant under
//      the +16-row and +32-row offsets used by the K1/V0/V1 copies, so
//      the fixed-offset issue addressing from db_addr is preserved.
//   2. __launch_bounds__(128, 6): ptxas must fit REG <= 85.
//      KILL CONDITION: LOCAL > 0 (spill) on any raced instance.
//
// Validation: swizzle is a self-inverse address bijection on our own
// buffers -- a wrong mapping produces garbage immediately, so the
// existing bit-compare vs db_full in tests is a complete check.
// ============================================================
...
constexpr int BR = 64;
constexpr int BC = 32;
constexpr int PAD = 0;   // swizzled layout: no padding, 128B rows
...
// Forward kernel
// ============================================================
#ifndef SWZ_MINBLOCKS
#define SWZ_MINBLOCKS 6   // build with -DSWZ_MINBLOCKS=1 to lift the REG squeeze (A/B)
#endif

template <int D, bool WRITE_L, bool FULL_TILES, int N_STATIC>
__global__ void __launch_bounds__(NWARPS * 32, SWZ_MINBLOCKS)
fa3swz_fwd_kernel(
    ...
    int N_runtime)
{
    ...
    // Swizzled destination: phys_chunk = (tid&7) XOR (r0&7). r1 = r0+16 and
    // the V block sits +32 rows, so (row&7) -- and thus the swizzle -- is
    // identical for all four copies: constant byte offsets still work.
    const uint32_t k0_off = (uint32_t)(r0 * ROW_BYTES)
                          + ((uint32_t)((tid & 7) ^ (r0 & 7)) << 4);
    const uint32_t k1_off = k0_off + 16 * ROW_BYTES;
    const uint32_t v0_off = k0_off + BC * ROW_BYTES;
    const uint32_t v1_off = k0_off + (BC + 16) * ROW_BYTES;
    ...
    issue_kv_fast(cur_base, 0);
    {
        ...
        for (int i = tid; i < total_h2; i += blockDim.x) {
            ...
            // swizzled column: chunk = c/8 -> chunk ^ (r&7), keep sub-chunk pos
            int pc = (((c >> 3) ^ (r & 7)) << 3) + (c & 7);
            if constexpr (FULL_TILES) {
                // N % BR == 0 guarantees every Q row in this block is valid.
                *reinterpret_cast<half2*>(&sQ[r][pc]) =
                ...
                *reinterpret_cast<half2*>(&sQ[r][pc]) = val;
            }
        }
        __syncthreads();
    }

    // Per-lane swizzle constant: all our ldmatrix rows satisfy row&7 == lane&7.
    const uint32_t quad16 = (uint32_t)(lane & 7) << 4;

    uint32_t qf[KSLICES][4];
    {
        const uint32_t qstage = smem_base + STAGE_BYTES;
        const uint32_t qrow = (uint32_t)(warp * 16 + (lane % 16)) * ROW_BYTES;
        const uint32_t qb16 = (lane < 16) ? 0u : 16u;
        #pragma unroll
        for (int ks = 0; ks < KSLICES; ks++) {
            uint32_t addr = qstage + qrow + (((uint32_t)(ks * 32) + qb16) ^ quad16);
            ldmatrix_x4(qf[ks][0], qf[ks][1], qf[ks][2], qf[ks][3], addr);
        }
    }
    __syncthreads();

    // Row parts of the ldmatrix addresses (chunk parts go through the XOR).
    const uint32_t qk_lane_base = (uint32_t)(lane & 7) * ROW_BYTES;
    const uint32_t qk_b16 = (lane < 8) ? 0u : 16u;
    const uint32_t pv_lane_base = (uint32_t)(BC + (lane & 15)) * ROW_BYTES;
...
// Host launchers
// ============================================================
static std::pair<torch::Tensor, torch::Tensor> fa3swz_forward_impl(
...
#endif
    if (want_L) {
        if (nstat == 4096)   launch(fa3swz_fwd_kernel<HD, true, true, 4096>);
        else if (nstat == 2048) launch(fa3swz_fwd_kernel<HD, true, true, 2048>);
        else if (full_tiles) launch(fa3swz_fwd_kernel<HD, true, true, -1>);
        else                 launch(fa3swz_fwd_kernel<HD, true, false, -1>);
    } else {
        if (nstat == 4096)   launch(fa3swz_fwd_kernel<HD, false, true, 4096>);
        else if (nstat == 2048) launch(fa3swz_fwd_kernel<HD, false, true, 2048>);
        else if (full_tiles) launch(fa3swz_fwd_kernel<HD, false, true, -1>);
        else                 launch(fa3swz_fwd_kernel<HD, false, false, -1>);
    }
...
std::vector<torch::Tensor> fa3swz_forward(torch::Tensor Q, torch::Tensor K, torch::Tensor V) {
    auto [O, L] = fa3swz_forward_impl(Q, K, V, /*want_L=*/true);
    return {O, L};
}

torch::Tensor fa3swz_forward_only(torch::Tensor Q, torch::Tensor K, torch::Tensor V) {
    auto [O, L] = fa3swz_forward_impl(Q, K, V, /*want_L=*/false);
    return O;
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &fa3swz_forward, "FA3 swz path (PAD=0 swizzle, 6 blocks/SM target): returns O half, L float");
    m.def("forward_only", &fa3swz_forward_only, "FA3 swz path, true O-only (SDPA API-latency comparison)");
}
